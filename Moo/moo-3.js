(async () => {
  ...
  const maxDelay = parseInt(prompt(`Максимальная задержка между кликами (в мс, по умолчанию ${defaultMaxDelay}):`, defaultMaxDelay), 10);
  ...
  let clicksDone = 0;
  let nextPauseAt = 850 + Math.floor(Math.random() * 300);
  ...
  for (let i = 0; i < clicksCount; i++) {
    ...
    if (clicksDone >= nextPauseAt && clicksDone < clicksCount) {
      const pauseDuration = 30000 + Math.random() * 20000;
      console.log(`Перерыв на ${Math.round(pauseDuration / 1000)} секунд`);
      await new Promise(resolve => setTimeout(resolve, pauseDuration));

      nextPauseAt += 850 + Math.floor(Math.random() * 300);
    }
  }
